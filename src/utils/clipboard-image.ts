...
function readClipboardImageViaPowerShell(): ClipboardImage | null {
	const tmpFile = join(tmpdir(), `hawcode-wsl-clip-${randomUUID()}.png`);
	...
	try {
		...
		const psScript = [
			...
			"$path = $env:HAWCODE_WSL_CLIPBOARD_IMAGE_PATH",
		...
		const result = runCommand("powershell.exe", ["-NoProfile", "-Command", psScript], {
			...
			env: { ...process.env, HAWCODE_WSL_CLIPBOARD_IMAGE_PATH: winPath },
		});
		if (!result.ok) {
			return null;
		}
