    (function() {
      ...
      function renderEntry(entry) {
        ...
        if (entry.type === 'message') {
          ...
          // Role label helper
          const roleLabel = (role, icon = '') => 
            `<span class="role-label role-${role}">${icon}${role}</span>`;

          if (msg.role === 'user') {
            let html = `<div class="user-message" id="${entryId}">${copyBtnHtml}${roleLabel('user', '> ')}${tsHtml}`;
          ...
          if (msg.role === 'assistant') {
            let html = `<div class="assistant-message" id="${entryId}">${copyBtnHtml}${roleLabel('assistant', '')}${tsHtml}`;
          ...
          if (msg.role === 'toolResult') {
            return `<div class="tool-execution ${msg.isError ? 'error' : 'success'}" id="${entryId}">${roleLabel('tool', '')}${tsHtml}<div class="tool-output"><pre>${escapeHtml(msg.content.filter(c => c.type === 'text').map(c => c.text).join('\n'))}</pre></div></div>`;
          }
        }
