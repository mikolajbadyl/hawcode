...
import type { LspManager, LspStatusPart } from "./lsp/lsp-manager.js";
...
export class FooterDataProvider {
	...
	private lspStatusUnsubscribe?: () => void;
	private lspStatusChangeCallbacks = new Set<() => void>();
	...
	/** Set the LSP manager for diagnostics status */
	setLspManager(manager: LspManager | undefined): void {
		if (this.lspStatusUnsubscribe) {
			this.lspStatusUnsubscribe();
			this.lspStatusUnsubscribe = undefined;
		}
		this.lspManager = manager;
		if (manager) {
			this.lspStatusUnsubscribe = manager.onStatusChange(() => {
				this.notifyLspStatusChange();
			});
		}
	}

	/** Get LSP status text for footer display (legacy single-string form) */
	getLspStatusText(): string {
		return this.lspManager ? this.lspManager.getStatusText() : "";
	}

	/** Structured LSP footer chips: one per active server. Empty when LSP is off or idle. */
	getLspStatusParts(): readonly LspStatusPart[] {
		return this.lspManager ? this.lspManager.getStatusParts() : [];
	}

	/** Whether LSP is enabled (even if no servers have spawned yet) */
	isLspEnabled(): boolean {
		return this.lspManager ? this.lspManager.isEnabled() : false;
	}

	/** Subscribe to LSP status changes */
	onLspStatusChange(callback: () => void): () => void {
		this.lspStatusChangeCallbacks.add(callback);
		return () => this.lspStatusChangeCallbacks.delete(callback);
	}

	private notifyLspStatusChange(): void {
		for (const cb of this.lspStatusChangeCallbacks) cb();
	}
	...
	dispose(): void {
		...
		this.lspStatusChangeCallbacks.clear();
	}
