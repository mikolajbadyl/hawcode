...
/**
 * Pluggable operations for the search tool.
...
export interface SearchToolOptions {
	/** Custom operations for search. Default: local filesystem plus ripgrep */
	operations?: SearchOperations;
}

function formatSearchCall(
	args: { pattern: string; path?: string; glob?: string } | undefined,
...
function formatSearchResult(
	...
	// Separate result lines from notices (notice block starts with [)
	...
	const maxLines = options.expanded ? resultLines.length : 15;
	const displayLines = resultLines.slice(0, maxLines);
	const remaining = resultLines.length - maxLines;
...
export function createSearchToolDefinition(
	...
	return {
		name: "search",
		label: "search",
		description: `Search file contents for a pattern using ripgrep. Returns matching lines with file paths and line numbers. Respects .gitignore. Use this instead of grep, rg, or ripgrep shell commands. Output is truncated to ${DEFAULT_LIMIT} matches or ${DEFAULT_MAX_BYTES / 1024}KB (whichever is hit first). Long lines are truncated to ${GREP_MAX_LINE_LENGTH} chars.`,
			...
			return new Promise((resolve, reject) => {
				...
				(async () => {
					try {
						...
						const ops = customOps ?? defaultSearchOperations;
